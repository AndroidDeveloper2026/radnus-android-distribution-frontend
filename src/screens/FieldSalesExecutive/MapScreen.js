// MapScreen.js - COMPLETE WITH GREEN ROUTE LINE
...
const MapScreen = ({ route }) => {
  ...
  const routeGeoJSON = useMemo(() => {
    ...
    // ✅ Filter out duplicate coordinates (same lat/lng)
  ...
  const routePoints = session?.pointCount ?? session?.route?.length ?? 0;
  ...
  return (
    <View style={ms.container}>
      ...
      >
        ...
        {/* ✅ Route Line - Green with White Outline */}
        {routeGeoJSON && (
          <>
            ...
            <MapLibreGL.ShapeSource id="routeSource" shape={routeGeoJSON}>
              ...
              <MapLibreGL.LineLayer
                id="routeLine"
                style={{ lineColor: '#22c55e', lineWidth: 4 }}  // ✅ Green line
              />
            </MapLibreGL.ShapeSource>

            {/* ✅ Route dots for visual clarity */}
        ...
        <MapLibreGL.PointAnnotation id="start" coordinate={startCoord}>
          <View style={ms.markerStart}>
            <MapPin size={22} color="#fc0404" strokeWidth={3.5} />
          </View>
        ...
        {!isActive && (
          <MapLibreGL.PointAnnotation id="end" coordinate={endCoord}>
            <View style={ms.markerEnd}>
              <MapPin size={22} color="#09ae03" strokeWidth={3.5} />
            </View>
          </MapLibreGL.PointAnnotation>
        )}
        ...
        <View style={ms.infoCard}>
          <View style={ms.row}>
            ...
            <Text style={ms.distance}>
              {formatDistance(session.totalDistanceKm)}
            </Text>
          </View>
...
//---------- Below code working completely - small issue -------------------
// import React, { useEffect, useMemo } from 'react';
// import { View, Text, StyleSheet, ActivityIndicator } from 'react-native';
// import MapLibreGL from '@maplibre/maplibre-react-native';
// import { Navigation, Clock, MapPin, CheckCircle, Timer } from 'lucide-react-native';
...
//   const isActive = session?.status === 'ACTIVE';
//   // Prefer the denormalized pointCount (kept accurate server-side even if a
//   // cached/partial session payload lacks the full route array), fall back
//   // to route.length for older payload shapes.
//   const routePoints = session?.pointCount ?? session?.route?.length ?? 0;
...
//           <MapLibreGL.PointAnnotation id="end" coordinate={endCoord}>
//             <View style={ms.markerEnd}>
//               <CheckCircle size={14} color="#fc0000" />
//             </View>
//           </MapLibreGL.PointAnnotation>
//         )}

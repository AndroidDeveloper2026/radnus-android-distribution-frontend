...
const FSEOnboarding = ({ navigation }) => {
  ...
  const handleFormSubmit = async (values, { setSubmitting: setFormikSubmitting }) => {
    ...
    try {
      ...
      console.error('Failed to submit FSE onboarding:', err);
      Alert.alert(
        'Submission Failed',
        err || 'Unable to submit this FSE for approval. Please check your connection and try again.',
        [
          { text: 'Cancel', style: 'cancel' },
          { text: 'Retry', onPress: () => handleFormSubmit(values, { setSubmitting: setFormikSubmitting }) },
        ],
      );
    } finally {
      setSubmitting(false);

...
const AdminDashboard = ({ navigation }) => {
  ...
  const fetchThisMonthSales = useCallback(async () => {
    ...
    try {
      const response = await api.get('/api/invoices?filter=month');
      const invoices = response.data || [];
      const total = invoices.reduce(
        (sum, inv) => sum + (inv.totalAmount || 0),
        0,
      ...
      console.error("Failed to fetch this month's sales:", error);
      setThisMonthSales(0);
    } finally {
      ...
      >
        ...
        <View style={styles.kpiRow}>
          ...
          <View style={styles.kpiBox}>
            ...
            {thisMonthSalesLoading ? (
              <ActivityIndicator size="small" color="#BB5cf6" />
            ) : (
              <Text style={styles.kpiValue}>₹{thisMonthSales}</Text>
            )}
          </View>
        </View>

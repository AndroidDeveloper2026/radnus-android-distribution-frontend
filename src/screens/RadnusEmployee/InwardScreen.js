...
const ICON_INWARD_STYLE = [styles.itemIcon, styles.iconInward];
const QTY_POSITIVE_STYLE = [styles.qtyValue, styles.positive];
...
// if one day has 200+ transactions, that entire day renders as one giant,
// non-virtualized block the instant it scrolls into view. That's the actual
// jank. We cap how many items render per day up front and let the user
// expand a day on demand, so no single row can ever be huge.
  ...
  return (
    <View style={styles.movementItem}>
      ...
      <View style={styles.itemQty}>
        <Text style={QTY_POSITIVE_STYLE}>+{item.qty}</Text>
...
const EmptyState = memo(({ hasActiveFilters }) => (
  <View style={styles.emptyState}>
    ...
    <Text style={styles.emptyTitle}>No inward records found</Text>
...
const InwardScreen = ({ navigation, route }) => {
  ...
  const products = useSelector((state) => state.products?.list || EMPTY_ARRAY);
  ...
  const rawItems = useMemo(() => {
    if (isInitialLoading || !products.length) return [];
  ...
  if (isInitialLoading) {
    return (
      <View style={styles.container}>
        <Header title="Stock Movement - Inward" showBackArrow navigation={navigation} />

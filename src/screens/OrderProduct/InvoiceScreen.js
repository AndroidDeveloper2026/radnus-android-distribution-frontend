...
const InvoiceScreen = ({ route }) => {
  ...
  const generatePDF = async () => {
    try {
      ...
      // ✅ After successful print, go back to EmployeeDashboard
      // Using replace removes the invoice screen from the stack
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['bottom']}>
      ...
      {/* Two buttons: Print & Home */}
      <TouchableOpacity style={[styles.fab, { right: 80 }]} onPress={generatePDF} disabled={printing}>
        <FileDown size={24} color="#FFFFFF" strokeWidth={2} />
        <Text style={styles.fabText}>{printing ? '...' : 'PDF'}</Text>
      </TouchableOpacity>

      {/* <TouchableOpacity style={[styles.fab, { right: 20, backgroundColor: '#2E7D32' }]} onPress={goToHome}>
